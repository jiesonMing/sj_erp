...
class DeclarationController extends BaseController
{
    ...
    public function oweList(){
        ...
        $list=db('owe')->alias('o')->join('erp_purchase p','p.id=o.purchaseId','left')->join('erp_goods g','g.id=o.goodsId','left')->field('o.oweId,o.oweCode,o.amount,o.inOwe,o.outOwe,o.createTime,o.status,o.remark,p.contractNum,g.goodsCode,g.goodsName')->where($where)->order('o.oweId asc')->paginate();
    ...
    public function batch(){
        ...
        $list=db('batch')->alias('b')->join('erp_warehouse w','w.id=b.wareId')->join('erp_shop s','s.shopId=b.shopId','left')->join('erp_goods g','g.id=b.goodsId','left')->field('b.batchId,b.batch,b.expireDate,b.type,b.amount,b.shelves,b.bSurStock,b.bUsingStock,b.status,b.remark,g.goodsCode,g.goodsName,w.warehouseName,w.warehouseType,s.shopName')->where($where)->order('b.batchId')->paginate();
    ...
    public function addBatch(){
        if(request()->isPost()){
            try{
                ...
                if($inwareGoods->outBatch>=$inwareGoods->qty){
                    $inwareGoods->status=80;
                }
                //更新入仓单的质检数量
            ...
            if($type==4){
                try{
                    ...
                    $owe->goodsId=$inwareGoods->goodsId;
                    $owe->amount=$outBatch;
                    $owe->inOwe=$outBatch;
                    if($inware->parentType==1){
                        ...
                        if($oldOwe->outOwe>=$oldOwe->amount){
                            $oldOwe->status=80;
                        }
                    }
                }catch(Exception $e){
            ...
            if($type!=4){
                try{
                    ...
                    $batch->goodsId=$inwareGoods->goodsId;
                ...
                try{
                    ...
                    if(!$inwareGoods->save(array(),$where)) exception('入仓商品可质检商品数量异常！');
                    if($type==2 || $type==3){
                        if(!$this->stockGoodsEdit($inwareGoods->goodsId,$outBatch,6)) exception('更新商品库存记录失败！');
                    }else{
                        if(!$this->stockGoodsEdit($inwareGoods->goodsId,$outBatch,4)) exception('更新商品库存记录失败！');
                    }
            ...
            $data=db('inwarehouse')->alias('i')->join('erp_inwarehouse_goods ig','ig.inwareId=i.inwareId and ig.inBatch>0','left')->join('erp_goods g','g.id=ig.goodsId','left')->join('erp_warehouse w','w.id=i.wareId','left')->field('i.inwareCode,ig.inwareGoodsId,g.goodsCode,g.goodsName,ig.inBatch,w.warehouseName')->where($where)->select();
            $wareName='';
            if($data){
    ...
    public function editBatch(){
        ...
        if(!$batchId) errorMsg(200,'批次信息有误，请刷新后重试！');
        $expire=request()->param('expire')?request()->param('expire'):'';
        if(!$expire) errorMsg(400,'保质期不能为空！');
        $amount=request()->param('amount/d')?request()->param('amount/d'):0;
        if($amount>0){//生成新的破损批次
            $oldBatch=Batch::get($batchId);
            $surStock=$oldBatch->shelves+$oldBatch->bSurStock;
            if($surStock<$amount) errorMsg(400,'当前批次可用库存数量少于转破损数量，请验证！');
            $owhere=array();
            $plus=$amount;
            if($oldBatch->shelves>0){
                if($oldBatch->shelves>=$plus){
                    $oldBatch->shelves=$oldBatch->shelves-$plus;
                    $owhere['shelves']=['>=',$plus];
                    $plus=0;
                }
                if($oldBatch->shelves<$plus){
                    $owhere['shelves']=['<',$plus];
                    $plus=$plus-$oldBatch->shelves;
                    $oldBatch->shelves=0;
                }
            }
            if($plus>0){
                if($oldBatch->bSurStock>=$plus){
                    $oldBatch->bSurStock=$oldBatch->bSurStock-$plus;
                    $owhere['bSurStock']=['>=',$plus];
                    $plus=0;
                }
                if($oldBatch->bSurStock<$plus) errorMsg(400,'当前批次可用库存数量少于转破损数量，请验证！');
            }
            if(!$owhere) errorMsg(400,'当前批次可用库存数量有误，请刷新重试！');
            $batch=new Batch;
            $batch->batch='B'.date('dHis').str_pad(rand(0,999999), 6, '0', STR_PAD_LEFT);
            $batch->companyId=$this->companyId;
            $batch->inwareId=$oldBatch->inwareId;
            $batch->wareId=$oldBatch->wareId;
            $batch->shopId=$oldBatch->shopId;
            $batch->goodsId=$oldBatch->goodsId;
            $batch->expireDate=$expire;
            $batch->type=2;
            $batch->amount=$amount;
            $batch->shelves=$amount;
            $batch->remark='从批次：'.$oldBatch->batch.'转过来的破损批次！';
            Db::startTrans();
            try{
                $batch->save();
                if(!$oldBatch->save(array(),$owhere)) exception('当前批次可用库存数量有误，请刷新重试！');
                if(!$this->stockGoodsEdit($oldBatch->goodsId,$amount,7)) exception('更新商品库存记录失败！');
                Db::commit();
                errorMsg(200,'done');
            }catch(Exception $e){
                Db::rollback();
